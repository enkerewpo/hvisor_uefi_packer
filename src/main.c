...
efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
  ...
  // CopyMem((void *)hvisor_bin_addr, (void *)hvisor_bin_start,
  // hvisor_bin_size);

  // Print(L"Copying hvisor dtb to 0x%lx, size is %d\n", hvisor_dtb_addr,
  //       hvisor_dtb_size);

  // CopyMem((void *)hvisor_dtb_addr, (void *)hvisor_dtb_start,
  // hvisor_dtb_size);

  // Print(L"Copying hvisor zone0 vmlinux binary to 0x%lx, size is %d\n",
  //       hvisor_zone0_vmlinux_addr, hvisor_zone0_vmlinux_size);
  // // print hvisor_zone0_vmlinux_start, hvisor_zone0_vmlinux_end
  // Print(L"vmlinux.bin start: 0x%lx, end: 0x%lx\n",
  // hvisor_zone0_vmlinux_start,
  //       hvisor_zone0_vmlinux_end);

  // CopyMem((void *)hvisor_zone0_vmlinux_addr, (void
  // *)hvisor_zone0_vmlinux_start,
  ...
  // // dump 20 instructions from hvisor_zone0_vmlinux_addr, 4 instructions per
  // // line
  // Print(L"Dumping instructions from vmlinux.bin 0x%lx\n",
  //       hvisor_zone0_vmlinux_addr);
  // for (int i = 0; i < 20; i++) {
  //   UINT32 *inst = (UINT32 *)(hvisor_zone0_vmlinux_addr + i * 4);
  //   // 4 instructions per line
  //   if (i % 4 == 0 && i != 0) {
  //     Print(L"\n");
  //   }
